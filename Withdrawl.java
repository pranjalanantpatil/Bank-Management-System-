...
public class Withdrawl extends JFrame implements ActionListener {
    ...
    Withdrawl(String pinnumber) {     //Constructor
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == withdraw) {
            String number = amount.getText();  //How much amount user can fill in this deposit field
            Date date = new Date(); // Creating date class object
            if(number.equals("")) {      //If user cannot fill the amount in the deposit field 
                JOptionPane.showMessageDialog(null, "Please enter the amount you want to Withdraw");   // Popup message
            } else {
                try{
                Conn conn = new Conn();  //Connection opening to dthe database
                String query = "insert into bank values('"+pinnumber+"', '"+date+"', 'Withdrawl', '"+number+"')";
                conn.s.executeUpdate(query);    //This is the DML command
